...
func TestNewChecker(t *testing.T) {
	_, err := newChecker(rootURL, 1, false, false, false, false)
	assert.Nil(t, err)
}

func TestNewCheckerError(t *testing.T) {
	_, err := newChecker(":", 1, false, false, false, false)
	assert.NotNil(t, err)
}

func TestNewCheckerWithMissingSitemapXML(t *testing.T) {
	_, err := newChecker("http://localhost:8081", 1, false, false, false, true)
	assert.NotNil(t, err)
}

func TestCheckerCheck(t *testing.T) {
	for _, s := range []string{rootURL, fragmentURL, baseURL, redirectURL} {
		c, _ := newChecker(s, 1, false, false, false, false)
...
func TestCheckerCheckMultiplePages(t *testing.T) {
	c, _ := newChecker(rootURL, 1, false, false, false, false)
...
func TestCheckerCheckPage(t *testing.T) {
	c, _ := newChecker(rootURL, 256, false, false, false, false)
